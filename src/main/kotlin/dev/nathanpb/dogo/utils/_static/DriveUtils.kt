package dev.nathanpb.dogo.utils._static
                    ...
                    override fun progressChanged(uploader: MediaHttpUploader?) {
                        when(uploader?.uploadState){
                            MediaHttpUploader.UploadState.INITIATION_STARTED -> {
                                dev.nathanpb.dogo.core.DogoBot.logger.info("Starting upload for ${file.name}")
                            }
                            MediaHttpUploader.UploadState.INITIATION_COMPLETE -> {
                                dev.nathanpb.dogo.core.DogoBot.logger.info("Upload for ${file.name} started!")
                            }
                            MediaHttpUploader.UploadState.MEDIA_IN_PROGRESS -> {
                                dev.nathanpb.dogo.core.DogoBot.logger.info("Upload for ${file.name}: ${uploader.progress}%")
                            }
                            MediaHttpUploader.UploadState.MEDIA_COMPLETE -> {
                                dev.nathanpb.dogo.core.DogoBot.logger.info("Upload for ${file.name} is done!")
                            }
                            MediaHttpUploader.UploadState.NOT_STARTED -> {
                                dev.nathanpb.dogo.core.DogoBot.logger.info("Upload for ${file.name}: NOT_STARTED")
                            }
                        }
                    }
                }
