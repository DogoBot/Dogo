...
class DogoData constructor(file : File){
    ...
    fun read() {
        ...
        json.keys().forEach { k -> map.put(k as String, json.get(k)) }
    }
    ...
    /**
     * Returns a Array<Any> from the data
     * @param key Value's key
     */
    fun getArray(key: String) : Array<Any>{
        val ar = getAny(key) as JSONArray
        val ar2 = ArrayList<Any>()
        for(i in 0..ar.length()-1){
            ar2.add(ar.get(i))
        }
        return ar2.toTypedArray()
    }
